...
public class FormEditCotragent extends VerticalLayout {
    ...
    public FormEditCotragent(ContractorService contractorService, DadataService dadata,
                             TypeOfContractorService typeOfContractorService,
                             ContractorGroupService contractorGroupService,
                             BankAccountService bankAccountService, TypeOfPriceService typeOfPriceService) {
...
// МОЙ НОВЫЙ КОД
            ...
            for (FormBankAccauntInner form : formsBankAccount) {
                ...
                for (FormBankAccauntInner formBankAccauntInner : formsBankAccount) {
                    bankAccountDto = formBankAccauntInner.getBankAccount();
                    if (!formBankAccauntInner.isDeleted() && formBankAccauntInner.isNewAccount()) {
                        contractorDto.getBankAccountDtos().add(bankAccountDto);
                    }
                    if (!formBankAccauntInner.isDeleted() && !formBankAccauntInner.isNewAccount()) {
                        for (BankAccountDto bankAccountOld : contractorDto.getBankAccountDtos()) {
                            if (bankAccountOld.getId() == bankAccountDto.getId()) {
                                bankAccountOld.setRcbic(bankAccountDto.getRcbic());
                                bankAccountOld.setBank(bankAccountDto.getBank());
                                bankAccountOld.setAddress(bankAccountDto.getAddress());
                                bankAccountOld.setCorrespondentAccount(bankAccountDto.getCorrespondentAccount());
                                bankAccountOld.setAccount(bankAccountDto.getAccount());
                                bankAccountOld.setMainAccount(bankAccountDto.getMainAccount());
                                bankAccountOld.setSortNumber(bankAccountDto.getSortNumber());
                            }
                        }
                    }
                }
                ...
                for (FormForFaceContactInner forFaceContactInner : formsFacesContact) {
                    contact = forFaceContactInner.getContactFace();
                    if (!forFaceContactInner.isDeleted() && forFaceContactInner.isNewFaceContact()) {
    ...
    private VerticalLayout getFormForContractorType(String type) {
        ...
        if (contractorDto.getLegalDetailDto().getCommentToAddress() != null)
            commentToAddressLegal.setValue(contractorDto.getLegalDetailDto().getCommentToAddress());
        inn = new TextField();
