...
public interface CustomerOrderApi {
    ...
    @GET("{url}")
    Call<List<CustomerOrderDto>> getAll(@Path(value = "url", encoded = true) String url);

    @Headers("Accept: application/json")
    @GET("{url}/{id}")
    Call<CustomerOrderDto> getById(@Path(value = "url", encoded = true) String url, @Path("id") Long id);
